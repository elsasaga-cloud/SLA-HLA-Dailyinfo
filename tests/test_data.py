...
class DataValidationTests(unittest.TestCase):
    def test_source_counts_dates_and_ohlc(self) -> None:
        for stock in GENERATOR.STOCKS:
            ...
            self.assertEqual("2026-05-19", target[0]["date"])
            self.assertEqual("2026-09-22", target[-1]["date"])
            self.assertEqual("2025-07-04", warmup[0]["date"])
            self.assertEqual("2026-05-18", warmup[-1]["date"])
            self.assertEqual(
