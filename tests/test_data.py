...
class DataValidationTests(unittest.TestCase):
    def test_source_counts_dates_and_ohlc(self) -> None:
        for stock in GENERATOR.STOCKS:
            ...
            self.assertEqual("2026-05-14", target[0]["date"])
            self.assertEqual("2026-09-17", target[-1]["date"])
            self.assertEqual("2025-07-01", warmup[0]["date"])
            self.assertEqual("2026-05-13", warmup[-1]["date"])
            self.assertEqual(
