...
class DataValidationTests(unittest.TestCase):
    def test_source_counts_dates_and_ohlc(self) -> None:
        for stock in GENERATOR.STOCKS:
            ...
            self.assertEqual("2026-05-20", target[0]["date"])
            self.assertEqual("2026-09-23", target[-1]["date"])
            self.assertEqual("2025-07-07", warmup[0]["date"])
            self.assertEqual("2026-05-19", warmup[-1]["date"])
            self.assertEqual(
